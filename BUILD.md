...
- `package.json`, `build.cjs` (build tools)
...
For GitHub distribution, use the `publish-dist.sh` script to create a separate branch:

```bash
./publish-dist.sh
git push origin dist-branch -f
```

Then install in Kiro using:
```
